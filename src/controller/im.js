import MD5 from '../../static/libs/md5';
import NIM from '../../static/libs/NIM_Web_NIM_weixin_v6.8.0';

import NetcallController from './netcall';
import { updateMultiPortStatus, dealMsg, showToast } from '../utils/util';
let app = getApp().globalData;
let store = getApp().globalData.store;
let orderCounter = 1; // 第一次进去onConnect onBlacklist onMutelist onFriends onMyInfo onUsers onTeams SyncDone onPushEvents
// 重连 onWillConnect
/* eslint-disable */
export default class IMController {
  constructor (headers) {
    getApp().globalData.nim = NIM.getInstance({
      // 初始化SDk
      // debug: true,
      appKey: getApp().globalData.ENVIRONMENT_CONFIG.appkey,
  ...
  */
  ...
  onFriends (friends) {
    ...
    if (getApp().globalData.ENVIRONMENT_CONFIG.openSubscription) {
      getApp().globalData.nim.subscribeEvent({
        type: 1,
        // 订阅用户登录状态事件
            ...
            success: res => {
              if (res.confirm) {
                ...
                if (currentPage.route.includes('videoCallMeeting')) {
                  ...
                  try {
                    // 兼容登录网关502错误离开房间
                    if (getApp().globalData.netcall) {
                      getApp().globalData.netcall.leaveChannel().then(() => {
                        getApp().globalData.netcall.destroy();
                  ...
                  try {
                    // 兼容登录网关502错误离开房间
                    if (getApp().globalData.netcall) {
                      getApp().globalData.netcall.hangup().then(() => {
                        getApp().globalData.netcall.destroy();
                        getApp().globalData.nim.destroy({
                          done: function () {
                            console.log('destroy nim done !!!');
  ...
  */
  ...
  onCustomSysMsg (sysMsg) {
    ...
    console.log(sysMsg); // 多端同步 正在输入自定义消息类型需要过滤
    ...
    if (id == 3) {
      ...
      if (currentPage.route.includes('videoCallMeeting') === false) {
        // 不在多人通话中，才提示
