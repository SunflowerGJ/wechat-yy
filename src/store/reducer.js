...
import dealGroupMsg from '../utils/dealGroupMsg';
/* eslint-disable */
...
let indexReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case 'FriendCard_Update_Initial':
      {
        // 初始化好友卡片
    ...
    case 'FriendCard_Update_InfoCard':
      {
        ...
        let card = action.payload; // 触发状态更新时friendCard可能为空
    ...
    case 'FriendCard_Delete_By_Account':
      {
        ...
        delete tempState.friendCard[account]; // todo 暂时先不删除，暂存删除好友前的在线状态
    ...
    case 'RawMessageList_Add_Msg':
      {
        ...
        */
        ...
        tempState.rawMessageList = Object.assign({}, tempState.rawMessageList); // 自己的退群消息就不记录、展示了
    ...
    case 'RawMessageList_Recall_Msg':
      {
        ...
        let msg = action.payload;
        let sessionId = msg.sessionId;
        tempState.rawMessageList = Object.assign({}, tempState.rawMessageList); // 替换原始文件消息，使得页面展示撤回消息提示
    ...
    case 'Del_Group':
      {
        let tempState = Object.assign({}, state);
        let group = action.payload;
        let sessionId = 'team-' + group.teamId; // 更新群信息

        tempState.groupList[group.teamId] = Object.assign({}, tempState.groupList[group.teamId], {
          isCurrentNotIn: true
        }); // 更新当前群信息
    ...
    case 'Update_Group_Member':
      {
        ...
        for (let i = 0, length = list.length; i < length; i++) {
          if (list[i].account === teamMember.account) {
            ...
            if (teamMember.teamId === tempState.currentGroup.teamId) {
              // 需要更新当前群
